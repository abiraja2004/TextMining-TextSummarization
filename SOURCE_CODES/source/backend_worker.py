import sys, time, getopt, select, socket
...
WORDTOVECMODELFILE = "/home/config/GoogleNews-vectors-negative300.bin.gz"
BACKENDPORT              = 6000
BACKENDHOST              = 'localhost'
BACKENDADDRESS           = (BACKENDHOST, BACKENDPORT)
BACKENDPASSWORD          = 'password'
BACKENDCONNECTION_FAMILY = 'AF_INET'


wordmodel = None

def init(w2v_vector_file=WORDTOVECMODELFILE):
  global wordmodel
  ...
  t = time.time()
  try:
    wordmodel = word2vec.Word2Vec.load_word2vec_format(WORDTOVECMODELFILE, binary=True)
    print wordmodel.most_similar(positive=['first'])
  except IOError, e:
...
def run_backend(replace=False, exit=False, w2v_vector_file=WORDTOVECMODELFILE, temp_startup_server=False):
  repr_req_count = 0
  ...
  if not temp_startup_server:
    try:
      ...
      conn = Client(BACKENDADDRESS, family=BACKENDCONNECTION_FAMILY, authkey=BACKENDPASSWORD)
  ...
  while not done:
    try:
      listener = Listener(BACKENDADDRESS, family=BACKENDCONNECTION_FAMILY, authkey=BACKENDPASSWORD)
